    ...
    <div className="cardContainer d-flex flex-column">
      <div className="card d-flex flex-row cardContainer2">
        <div className="card-group imageContainer">
          <img
            src={`https://image.tmdb.org/t/p/w500/${productDetails?.backdrop_path}`}
            className="card-img-start mt-1"
            alt=""
          />
        </div>
        ...
        <div className="card-body fw-bolder infoContainer">
          <div className="d-flex flex-row justify-content-between">
            <h5 className="card-title fw-bolder fs-2 m-0 mt-3 ">
          ...
          <h6 className="text-secondary fs-6 pb-4">
            {productDetails?.release_date}
          </h6>
          <Stars
            rate={productDetails?.vote_average}
            count={productDetails?.vote_count}
          />

          <p className="card-text fs-6 fw-bolder pt-2">
            {productDetails?.overview}
          </p>
          {productDetails?.genres?.map((gen, index) => {
            return (
              <span className="btn btn-warning my-4 py-0 px-3 m-1">
                {gen.name}
              </span>
            );
          })}
          <div className="d-flex flex-row">
            <h6 className="pe-5">
              ...
              <span className="text-secondary">{productDetails?.runtime} </span>
              Min
            </h6>
            <h6 className="ps-5">
              <strong>Languages: </strong>
              <span>{productDetails?.original_language}</span>
            </h6>
          </div>
          ...
          <a
            href={productDetails?.homepage}
            target="blank"
            className="btn btn-light border border-success my-4 py-0 px-3 m-1"
          >
            website
          </a>
        </div>
